...
public:
    ...
    std::vector<result> run(int dimension, int testFunction, double boundaryLow, double boundaryUp)
    {
        ...
        for (int i = 0; i < iterations; i++)
        {
            ...
            {
                std::vector<Agent> twoDistinct = get2distinct(population, population[j]);
                ...
                for (int d = 0; d < dimension_; d++)
                {
                    ...
                    {
                        //verze UNIQUE/1/bin
                        y.push_back(dimensionMove(mostUnique.position[d], twoDistinct[0].position[d], twoDistinct[1].position[d]));
                    }
                    // otherwise set yi = xi
                    else
                    {
