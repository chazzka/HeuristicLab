/*
    novelty bude nasazeno v případě že se dlouho opakují stejné či podobné výsledky, 
    bude vypnuto až se přestanou opakovat
*/
#include <vector>
    ...
    std::vector<result> run(int dimensionsCount, int testFunction, double boundaryLow, double boundaryUp)
    {
        ...
        for (int g = 0; g < generations; g++)
        {
            ...
            for (int i = 0; i < population.size(); i++)
            {
                ...
                //D - vysledky nejsou stejne, vypiname novelty
                else
                {
                    novelty = 0;
                }
