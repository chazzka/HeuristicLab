/*
    pohyb jedince se provede 10x a přijme se pohyb, který vytvoří nejunikátnější částici
*/
#include <vector>
...
public:
    ...
    void evaluateRoGetMostUnique(std::vector<Particle> &population, Particle &mostUnique, std::vector<std::vector<double>> &positions, int popSize)
    {
        ...
        {
            population[i].ro = utils::getRo(population[i].positionXi, positions, popSize);
    ...
    std::vector<result> run(int dimensionsCount, int testFunction, double boundaryLow, double boundaryUp)
    {
        ...
        for (int g = 0; g < generations; g++)
        {
            for (int i = 0; i < population.size(); i++)
            {
                ...
                std::vector<Particle> tempPopulation;
                ...
                for (int j = 0; j < 10; j++)
                {
                    ...
                    {
                        dimensionMove(tempParticle, gBestParticle, d);
                    }
                    tempPopulation.push_back(tempParticle);
                }

                Particle tempMostUnique = tempPopulation[0];
                evaluateRoGetMostUnique(tempPopulation, tempMostUnique, tempPositions, 10);

                currentParticle = tempMostUnique;
