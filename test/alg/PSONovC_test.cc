...
TEST(evaluateRoGetMostUnique, isMostUniqueReallyMostUnique)
{
    ...
    {
        psoNovC.evaluateRoGetMostUnique(population, mostUnique, positions);
    }
    ...
    for (auto &p : population)
    {
        if (p.bestRo > testBest.bestRo)
        {
            testBest = p;
        }
    }

    ASSERT_EQ(mostUnique.positionXi, testBest.positionXi);
    ASSERT_EQ(mostUnique.bestRo, testBest.bestRo);
}
